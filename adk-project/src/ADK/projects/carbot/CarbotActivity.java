...
public class CarbotActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		clockwise = (Button) findViewById(R.id.button3);
		counterClockwise = (Button) findViewById(R.id.button4);
		pTurnLeft = (Button) findViewById(R.id.button5);
		pTurnRight = (Button) findViewById(R.id.button6);
    }
	...
	public void forward (View V){
		if(!forward.isActivated())
			sendCommand((byte)1);
		else sendCommand((byte)0);
	}
	public void reverse (View V){
		if(!reverse.isActivated())
			sendCommand((byte)2);
		else sendCommand((byte)0);
	}
	public void clockwise (View V){
		if(!clockwise.isActivated())
			sendCommand((byte)3);
		else sendCommand((byte)0);
	}
	public void cclockwise (View V){
		if(!counterClockwise.isActivated())
			sendCommand((byte)4);
		else sendCommand((byte)0);
	}
	public void pointTurnR (View V){
		if(!pTurnRight.isActivated())
			sendCommand((byte)5);
		else sendCommand((byte)0);
	}
	public void pointTurnL (View V){
		if(!pTurnLeft.isActivated())
			sendCommand((byte)6);
		else sendCommand((byte)0);
